"""
Functions : Optimization Model 3 functions
...
def write_allocation(allocation,rank, lect_count):
    workbook = xlsxwriter.Workbook('Results/M3_output_alloc_lteX.xlsx')
    worksheet1 = workbook.add_worksheet('Model3_alloc')
    row = 0
    for col, data in enumerate(allocation.T):
        worksheet1.write_column(row,col, data)
    #Add worksheet of allocation ranks
    worksheet2 = workbook.add_worksheet('Model3_alloc_ranks')
    row = 0
    col = 0
    for ranks in rank:
    ...
    worksheet3 = workbook.add_worksheet('Model3_alloc_lect_count')
    row = 0
    col = 0
